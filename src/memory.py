...
class ReplayBuffer:
    def __init__(self, state_dim=4, capacity=1e5):
        capacity = int(capacity)
        state_dim = int(state_dim)
        
        self._state_buffer = np.zeros([capacity, state_dim], dtype=np.float64)
        self._action_buffer = np.zeros([capacity], dtype=np.float32)
        self._next_state_buffer = np.zeros([capacity, state_dim], dtype=np.float64)
        self._reward_buffer = np.zeros([capacity], dtype=np.float64)
