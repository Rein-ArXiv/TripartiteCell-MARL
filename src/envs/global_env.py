...
class Cellenv:
    def __init__(self, time_interval=1, islet_num=1, max_time=200, glucose_fix=False):
        ...
        self.action_space = spaces.MultiDiscrete(np.full((self.islet_num, 3)))
        self.state = None
    ...
    def reset(self, random_init=True, glucose_level=None, seed=None):
        self.curr_time = 0
        ...
        if random_init:
            for i in range(self.islet_num):
                ...
                cell_amp = np.random.uniform(0.1, 0.5, 3)
            ...
            glucose_column = np.full((self.state.shape[0], 1), glucose_init)
